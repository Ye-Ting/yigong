...
<div class='panel'>
  ...
  <div class='panel-actions'><?php echo html::a($this->inlink('create', "type={$type}&category={$categoryID}"), '<i class="icon-plus"></i> ' . $lang->activity->create, 'class="btn btn-primary"');?></div>
  <?php endif;?>
  </div>
  <table class='table table-hover table-striped tablesorter'>
    <thead>
      <tr>
        ...
        <?php if($type != 'page'):?>
        <th class='text-center w-200px'><?php commonModel::printOrderLink('category', $orderBy, $vars, $lang->activity->category);?></th>
        <?php endif;?>
        <th class='text-center w-160px'><?php commonModel::printOrderLink('addedDate', $orderBy, $vars, $lang->activity->addedDate);?></th>
        <th class='text-center w-60px'><?php commonModel::printOrderLink('views', $orderBy, $vars, $lang->activity->views);?></th>
    ...
    <tbody>
      ...
      <tr>
        ...
        <?php if($type != 'page'):?>
        <td class='text-center'><?php foreach($activity->categories as $category) echo $category->name . ' ';?></td>
        <?php endif;?>
        <td class='text-center'><?php echo $activity->addedDate;?></td>
        <td class='text-center'><?php echo $activity->views;?></td>
