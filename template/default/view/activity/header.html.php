<?php
/**
 * The common/header file of activity module of chanzhiEPS.
 *
...
<body>
<div class='page-container page-activity'>
  ...
  <nav id="activityNav" class="navbar navbar-default" role="navigation">
    <div class='wrapper'>
      <ul class="nav navbar-nav">
        <li <?php if(empty($category)) echo "class='active'"?>>
           <?php echo html::a($this->inlink('index'), (isset($this->config->site->type) and $this->config->site->type == 'activity') ? $lang->home : $lang->activity->home)?>
        </li>
        <?php 
        foreach($navs as $nav)
        {
          isset($category->id) ? $categoryID = $category->id : $categoryID = 0;
          $class = $nav->id == $categoryID ? "class='nav-activity-$nav->id active'" : "class='nav-activity-$nav->id'";
          echo "<li {$class}>" . html::a($this->inlink('index', "id={$nav->id}", "category={$nav->alias}"), $nav->name) . '</li>';
        }
        ?>
      </ul>
      <?php if(!isset($this->config->site->type) or $this->config->site->type != 'activity'):?>
      <ul class="nav navbar-nav navbar-right">
        <li><?php echo html::a($config->webRoot, '<i class="icon-home icon-large"></i> ' . $lang->activity->siteHome);?></li>
      </ul>
      <?php endif;?>
    </div>
  </nav>
